#!/bin/sh
cd .. # goes back out of my automation folder so i can access the expermients folder
runs=$2
experiment = $1
...
do
	temp="experiments/$output_folder/external_simulation_${random_seed}.argos"# appends the random seed to this so i get a different one each time i run it
